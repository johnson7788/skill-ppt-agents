...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  启动 evidence-a2ui 循证问答 (MOCK 离线)${NC}"
echo -e "${GREEN}========================================${NC}"

# 启动后端（EVIDENCE_MOCK=1 读 fixtures/loratadine.json，不触网、不调 LLM）
echo -e "${YELLOW}启动后端 (端口 8700, MOCK)...${NC}"
cd "$PROJECT_DIR/backend"
EVIDENCE_MOCK=1 uv run python server_evidence.py &
BACKEND_PID=$!
